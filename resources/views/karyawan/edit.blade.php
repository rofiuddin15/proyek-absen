  ...
  <div class="row">
    <div class="col-xl">
      <div class="card mb-2">
        ...
        <div class="card-body">
          <form method="POST" action="{{ route('karyawan.update', ['karyawan' => $profile->user_id])}}">
            @csrf
            ...
            <div class="mb-3 row">
              <div class="col-md">
                ...
                <input type="text" name="first_name" class="form-control" id="first_name" value="{{$profile->first_name}}" placeholder="Tulis nama depan" required/>
              </div>
              <div class="col-md">
                <label class="form-label" for="last_name">Nama Belakang</label>
                <input type="text" name="last_name" class="form-control" id="last_name" value="{{$profile->last_name}}" placeholder="Tulis nama belakang jika ada" />
              </div>
            </div>
            <div class="mb-3 row">
              <div class="col-md">
              ...
              <div class="col-md">
                  ...
                  <input class="form-control" name="phone" type="text" value="{{$profile->phone}}" id="phone"/>
              </div>
            </div>
            <div class="mb-3">
              <label for="address" class="col-md-2 col-form-label">Alamat</label>
              <input class="form-control" name="address" value="{{$profile->address}}" type="text" id="address" />
            </div>
            <div class="mb-3">
              <label for="email" class="col-md-2 col-form-label">E-mail*</label>
              <input class="form-control" name="email" value="{{$profile->user->email}}" type="email" id="email" required/>
            </div>
            <div class="mb-3 row">
              <div class="col-md">
                <label class="form-label" for="username">No Pegawai*</label>
                <input type="text" name="name" class="form-control" id="username" value="{{$profile->user->name}}" placeholder="Tulis no pegawai" required/>
              </div>
              <div class="col-md">
              ...
              <div class="col-md">
                ...
                <select name="shift[]" class="form-select" id="multiple-select" data-placeholder="Pilih shift" multiple required>
                  ...
                  @foreach ($shift as $item)
                      <option value="{{ $item->id }}" {{ $item->id == in_array($item->id, $userShift) ? 'selected' : '' }}>{{ $item->name }}</option>    
                  @endforeach
              </select>
              </div>
              <div class="col-md">
